...
class Block(nn.Module):
    ...
    def __init__(self, dim, dim_out):
        super().__init__()
        self.proj = nn.Conv1d(dim, dim_out, 1)#, padding=1)
...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = map(
            lambda t: rearrange(t, 'b (h c) l -> b h c l', h=self.heads),
            qkv)
        
        q = q.permute(0, 1, 3, 2) # q: (b, h, l, c)

        out = torch.matmul(
            v, 
            F.softmax(torch.matmul(q, k) / self.scale, dim=-1).permute(0, 1, 3, 2)
        ) 
...
class Unet(nn.Module):
    ...
    def __init__(self,
        ...
        self.init_conv = nn.Conv1d(in_dim, init_dim, 1)
