...
gino_model_filename = "vbllgino_model.pt"
fm_model_filename = "fm_model.pt"

gino_model_filepath = os.path.join(model_dir, gino_model_filename)
fm_model_filepath = os.path.join(model_dir, fm_model_filename)

pgino_model.load_state_dict(torch.load(gino_model_filepath, weights_only=False), strict=False)
fm_model.load_state_dict(torch.load(fm_model_filepath, weights_only=False), strict=True)

print(f"Loaded VBLLGINO model from {gino_model_filepath} and FM model {fm_model_filepath}")
...
with torch.no_grad():
    for batch in tqdm(val_loader):
        ...
        lr_context = lr_output.predictive.sample((n_sample,))
        ...
        for _ in range(NOISE_SAMPLES):
            ...
            HR_OUTS.append(rearrange(hr_out, 'n 1 x y -> 1 n x y'))
        
        HR_OUTS = torch.cat(HR_OUTS, dim=0)
...
pred_filename = 'FMVBLLGINO-PREDS-AUEU.npz'
...
# SHAPE -> (NUM_TEST_SAMPLES, NUM_NOISE_SAMPLES, NUM_MODEL_SAMPLES, X, Y)
# EU -> VAR(AXIS=-3).MEAN(AXIS=-3)
# AU -> MEAN(AXIS=-3).VAR(AXIS=-3)