...
# model.load_state_dict(torch.load(os.path.join(model_dir, f"model.pt"), weights_only=False), strict=False)

param_list = [
    {'params': model.feature_module.parameters(), 'weight_decay': 1e-4},
    {'params': model.vb_layer.parameters(), 'weight_decay': 0.0}
]

optimizer = torch.optim.AdamW(param_list, lr=1e-3)
...
for epoch in pbar:
    ...
    for batch in train_loader:
        ...
        # Calculate loss
        # loss = loss_fn(output[0], target[:,None].to(device))
        loss = output.train_loss_fn(target_data.to(device))
